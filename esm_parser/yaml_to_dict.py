...
def check_changes_duplicates(yamldict_all, fpath):
    ...
    add_note = "Note that multiple ``add_<variable>`` in a single file are compatible " \
    ...
    for yamldict in yamldict_all.values():
        ...
        changes_list = esm_parser.find_key(yamldict, "_changes", "add_",
                                           paths2finds = [], sep=",")
        add_list = esm_parser.find_key(yamldict, ["add_"], "",
                                           paths2finds = [], sep=",")
        ...
        changes_types = set([y for x in changes_list for y in x.split(",")
                             if "_changes" in y])
        # Find ``add_`` types
        add_types = set([y for x in add_list for y in x.split(",")
                             if "add_" in y])
